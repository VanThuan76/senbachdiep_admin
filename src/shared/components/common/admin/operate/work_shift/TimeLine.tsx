        ...
        <div className='mt-4 flex flex-col items-start justify-start gap-8'>
          <div className='ml-20 flex h-full w-[1206px] items-center justify-start'>
            {timeLineWorkShifts.map(time => (
              <p key={time} className='!w-[67px]'>
                {time}
              </p>
            ))}
          </div>
          {fakeData.data.zone.single.map((item: { [key: string]: any }, index: number) => (
            <div key={index} className='border-b-bg-[#FCFCFC] flex w-full items-center justify-start border-b'>
              <div className='min-w-[60px] rounded-md bg-[#D9A536] px-2 py-4 text-center'>D0{index + 1}</div>
              <div className='ml-5 flex w-full items-center justify-start gap-2 overflow-hidden'>
                {(
                  ...
                  <div
                    key={childIndex}
                    className={cn('rounded-md border-2 border-solid border-[#C9C9C9] px-2 py-4 text-center', `w-full`)}
                  >
